...
def train(args, train_dataset, model, tokenizer):
    ...
    if args.restore:
        if os.path.exists(os.path.join(args.output_dir, 'global_step.bin')):
            ...
            if args.tpu:
                print("Load from output_dir {}".format(output_dir))
            else:
            ...
            if args.tpu:
                print("Start from global_step={} epoch={} step={}".format(global_step, start_epoch, start_step))
            else:
    ...
    tacred_model.zero_grad(set_to_none=True)
    ...
    for epoch in range(start_epoch, int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            if args.tpu:
                print("Epoch {}/{} - Iter {} / {}, loss = {:.5f}, time used = {:.3f}s".format(epoch, int(args.num_train_epochs),step,
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.tpu:
                        print("Saving model checkpoint, optimizer, global_step to %s", output_dir)
                        print("Saving model checkpoint, optimizer, global_step to %s", output_dir)
                    else:
...
def evaluate(args, model, tokenizer, prefix="", epoch=0, global_step=0):
    ...
    for dataset_type in ['dev', 'test']:
        ...
        for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
            ...
            if args.tpu:
                print('{} result:{}'.format(dataset_type, result))
            else:
...
# TODO: datasets.load_dataset('xiaobendanyn/tacred')
def load_and_cache_examples(args, task, tokenizer, dataset_type, evaluate=False):
    ...
    if output_mode == "classification":
        all_label_ids = torch.tensor(np.array([f.label_id for f in features]), dtype=torch.long)
    elif output_mode == "regression":
        all_label_ids = torch.tensor(np.array([f.label_id for f in features]), dtype=torch.float)
    all_subj_special_start_ids = torch.tensor(np.array([f.subj_special_start_id for f in features]), dtype=torch.float)
    all_obj_special_start_ids = torch.tensor(np.array([f.obj_special_start_id for f in features]), dtype=torch.float)
