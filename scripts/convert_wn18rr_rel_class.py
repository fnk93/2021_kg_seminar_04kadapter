...
def convert_wn18rr() -> None:
    in_path = pathlib.Path('data/WN18RR/original')
    out_path = pathlib.Path('data/WN18RRrel')
    if not out_path.exists():
        out_path.mkdir()
    convert_data(path=in_path, out_path=out_path, file='test')
    convert_data(path=in_path, out_path=out_path, file='valid', new_file='dev')
...
def main() -> None:
    convert_wn18rr()
    with open(BASE_PATH / 'wn18rr-relations.json', 'w+', encoding='utf-8') as fw:
